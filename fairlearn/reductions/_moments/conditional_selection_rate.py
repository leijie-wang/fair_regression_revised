...
class ConditionalSelectionRate(ClassificationMoment):
    ...
    and vice versa by measuring the difference betwen error(A=a) * ratio and total_error, and error(A=a) and total_error * ratio
    The value of `ratio` has to be in the range (0,1] with smaller values
    corresponding to weaker constraint. 
    The `ratio` equal to 1 corresponds to the constraint where error(A = a) = total_error
    """

    #not very clear about the function of ratio.
    ...
    def default_objective(self):
        ...
        #that is, minimization of classfication error.
        return ErrorRate()
    ...
    def load_data(self, X, y, event=None, utilities=None, **kwargs):
        ...
        self.tags[_EVENT] = event #add a column whose label is “event” and which specifies the event of each instance.
        if utilities is None:
            utilities = np.vstack([np.zeros(y.shape, dtype=np.float64),
                                  np.ones(y.shape, dtype=np.float64)]).T
        #in the demographic parity case, g(X,A,Y,h(X))=h(X), so its utilities are simply such a matrix 
        self.utilities = utilities

        self.prob_event = self.tags.groupby(_EVENT).size() / self.total_samples #the probability of each event, the type of which is also a DataFrame.
        self.prob_group_event = self.tags.groupby(
            [_EVENT, _GROUP_ID]).size() / self.total_samples 
            #"group_id" represents sensitive features, but what is the difference between this and event. 
        signed = pd.concat([self.prob_group_event, self.prob_group_event],
                           keys=["+", "-"],
                           names=[_SIGN, _EVENT, _GROUP_ID])
        self.index = signed.index
        #here index is the cartesian product of sign, event, and sensitive features
        self.default_objective_lambda_vec = None #why does this mean? seems to relate to whether the norm of lambda is equal to B or less than B.

        # fill in the information about the basis
        event_vals = self.tags[_EVENT].dropna().unique() #a numpy array
    ...
    #unclear about the predictor and the utilities 
    #the predictor predicts a classification for each X.
    def gamma(self, predictor):
        """Calculate the degree to which constraints are currently violated by the predictor."""
        utility_diff = self.utilities[:, 1] - self.utilities[:, 0]
        pred = utility_diff.T * predictor(self.X) + self.utilities[:, 0] 
        #the multiplication of utility_diff.T and predictor(self.X) is the multiplication of the corresponding entries.
        ...
        expect_group_event[_LOWER_BOUND_DIFF] = - expect_group_event[_PREDICTION] \
            + self.ratio * expect_event[_PREDICTION] #Why are we going to use a ratio that is not one?
    ...
    def bound(self):
        ...
        #eps for each group_id and each sign (negative and positive)

    #unclear about lambda, why do we project lambdas as follows?
    ...
    def signed_weights(self, lambda_vec):
        ...
        ) #agree with the cost of predicting 1 for xi in the case of demographic parity
          #also with that in the case of equalized odds and equalized opportunities 
          #(these two differ only to the extent that the event "label=0" in the former case becomes NaN in the latter case)
        utility_diff = self.utilities[:, 1] - self.utilities[:, 0]
...
class TruePositiveRateDifference(ConditionalSelectionRate):
    ...
    def load_data(self, X, y, **kwargs):
        ...
        super().load_data(X, y,
                          ...
                          **kwargs) #when y = 1, the event is "label=1", else event is NaN

class DemographicParity_Theta(ConditionalSelectionRate):
    short_name = "DemographicParity_Theta"
    def __init__(self):
        super().__init__()

    def load_data(self,X,y,**kwargs):
        super().load_data(X,y,event=X["theta"],**kwargs)
