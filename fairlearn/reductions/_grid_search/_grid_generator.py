...
class _GridGenerator:
    ...
    def __init__(self, grid_size, grid_limit, pos_basis, neg_basis, neg_allowed, force_L1_norm,
        ...
        #here the dim represents the number of constraints according the definition of pos_basis
        ...
        #here the index is the cartesian product of signs, events, and group id.
        else:
            self.grid_offset = grid_offset #here we have a requirement for the type of grid_offset
        ...
        #unclear about its meanings
        #neg_allowed is an array whose elements represent whether negative values are allowed for each lambda
    ...
    def accumulate_integer_grid(self, index, max_val):
        if index == self.dim:
            ...
            if (index == self.dim - 1) and (self.force_L1_norm):
                ...
                values = range(min_val, max_val + 1)#why add an extra one?Because we only consider the integer grid.
            #Isn't values a local variable???
