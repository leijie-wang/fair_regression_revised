...
class GridSearch(BaseEstimator, MetaEstimatorMixin):
    ...
    :type grid_size: int
    #the number of lambda parameters
    ...
    def fit(self, X, y, **kwargs):
        ...
        for i in grid.columns:
            ...
            if is_classification_reduction:
                ...
                y_reduction = y_train#with no adjustments of true labels.
            ...
            self.objectives_.append(objective.gamma(predict_fct)[0])#error for this classifiers
            self.gammas_[i] = self.constraints.gamma(predict_fct)#violation of constraints
        ...
        if self.selection_rule == TRADEOFF_OPTIMIZATION:
            def loss_fct(i):
                return self.objective_weight * self.objectives_[i] + \
                    ...
                    #why pick up the max gamma? we use max{demographic parity} to measure the fairness
