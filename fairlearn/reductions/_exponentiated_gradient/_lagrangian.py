...
class _Lagrangian:
    ...
    #probably related to the best response of the Q player, that is, the oracles of the cost-sensitive classification problem
    ...
    def __init__(self, X, sensitive_features, y, estimator, constraints, eps, B,opt_lambda=True,error_weights=None):
        
        self.X = X
        self.n = self.X.shape[0]
        self.constraints = constraints
        self.constraints.load_data(X, y, sensitive_features=sensitive_features)
        #the definition is in moment.py, which initializes the basic value of constraints.
        self.obj = self.constraints.default_objective()
        #the minimization of prediction error
        self.obj.load_data(X, y, sensitive_features=sensitive_features,weights=error_weights)
        #this is seen as a moment

        self.pickled_estimator = pickle.dumps(estimator)
        #serialize the estimater into a byte object, but where is the estimater definition???
        self.eps = eps #bounds for constraint violations
        self.B = B #bounds for lambda
        self.opt_lambda = opt_lambda
        #confused about this

        self.hs = pd.Series(dtype="float64")
        #probably the series of h for every turn
    ...
    def _eval(self, Q, lambda_vec):
        ...
        if callable(Q):
            ...
            #return the error of this prediction
            gamma = self.constraints.gamma(Q)
            #return the violation of constraints for each group_id and event.
        else:
        ...
        if self.opt_lambda:
            ...
            #only consider the positive one (lambda_pos-lambda_neg) or (lambda_neg-lambda_pos). but why???
        else:
            L = error + np.sum(lambda_vec * gamma) - self.eps * np.sum(lambda_vec)
        #calculate the projected lambda, and then calculate the new value of L
    ...
    #unclear about why do we need this?
    def eval_gap(self, Q, lambda_hat, nu):
        ...
        result = _GapResult(L, L, L_high, gamma, error) #the max value between (L - L_low) and (L_high - L), where L_low is made equal to L. 
        for mul in [1.0, 2.0, 5.0, 10.0]:
            ...
            if result.gap() > nu + _PRECISION:
                break #why do we break at this point???
        return result
    ...
    def solve_linprog(self, nu):
        ...
        #In this case, there is no new classifier obtained in the round, so the best resul is the same as the last one.
        c = np.concatenate((self.errors, [self.B]))#append the self.B to the arrays of errors.
        A_ub = np.concatenate((self.gammas - self.eps, -np.ones((n_constraints, 1))), axis=1)
        #we can see here that the dimension of self.gammas is n_constraints \times n_hs, so in the best_h function, we transpose self.gammas.
        b_ub = np.zeros(n_constraints) #for the combination of classifiers, its violation of constraints is less than zero.
        A_eq = np.concatenate((np.ones((1, n_hs)), np.zeros((1, 1))), axis=1)
        b_eq = np.ones(1) #the sum of weights for classifiers is one.
        result = opt.linprog(c, A_ub=A_ub, b_ub=b_ub, A_eq=A_eq, b_eq=b_eq, method='simplex')
        Q = pd.Series(result.x[:-1], self.hs.index)
        #Why add one additional classifier with a fixed error and gamma.
        ...
        #unclear about the langrange dual problems

        self.last_linprog_n_hs = n_hs
        self.last_linprog_result = (Q, lambda_vec, self.eval_gap(Q, lambda_vec, nu))
        return self.last_linprog_result

    #the reduction to a weighted classification problem
    def _call_oracle(self, lambda_vec):
        signed_weights = self.obj.signed_weights() + self.constraints.signed_weights(lambda_vec)
        #here, it reduces the best response problem to a weighted classification problem, where the weight is actually C_0 - C_1.
        #so it is true that second part is the negative of the part of C_1, and the first part is 1 for y_i = 1 and -1 for y_i = 0.
        redY = 1 * (signed_weights > 0)#if weights = C_0 - C_1 >0, then the true label is 1, else 0.
        redW = signed_weights.abs()#according to the requirement of weighted classifcation problem
        redW = self.n * redW / redW.sum()#do factoring, but why multiplied by n???

        redY_unique = np.unique(redY)#in the normal case, the answer is {0,1}
        ...
        return classifier
    #the solution to the best response problem of the player h.
    ...
    def best_h(self, lambda_vec):
        ...
        #isn't here we solve call the classification algorithm twice?

        if not self.hs.empty:
            values = self.errors + self.gammas.transpose().dot(lambda_vec)
            #the L_value of each classifer until now.
        ...
        if h_value < best_value - _PRECISION:
            ...
            best_idx = h_idx #the value of lambda_vec is not constant, so the best value also changes.

        #if h_value is not satisfying, simply return the best one in the history of classifiers.
