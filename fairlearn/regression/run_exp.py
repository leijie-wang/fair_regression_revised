...
def subsample(x, a, y, size, random_seed=DATA_SPLIT_SEED):
    ...
    """
    toss = 1 - size / (len(x))
    ## size indicates the number of examples in the train set.
...
def square_loss_benchmark(dataset, n):
    ...
    if _SMALL:
        ...
        ## only run these two benchmarks on a large dataset.
...
#                           random_seed=DATA_SPLIT_SEED)
result_list = [result1]
read_result_list(result_list)  # A simple print out for the experiment
       
# base_solver1 = solvers.RF_Base_Regressor()
# base_result1 = base_train_test(dataset, size, base_solver1, loss="logistic",
#                     random_seed=DATA_SPLIT_SEED)

# # base_result_list = [base_result1]
# # plot.disp_curve_list(result_list,base_result1)
